...
function setup() {
  var info = document
    .getElementById("upload-image-area")
...
// Custom Network Mode Section
const customModelButton = document.querySelector("#custom-network-model");
const closeAllButton = document.querySelector(
  ".upload-custom-weight .close-all"
);
const customNetworkSection = document.querySelector(".custom-model-section");
console.log(closeAllButton);
customModelButton.addEventListener("click", openCustomModelOption);
closeAllButton.addEventListener("click", openCustomModelOption);
function openCustomModelOption(e) {
  if (!isOpenModelOption) {
    customNetworkSection.style.display = "flex";
    isOpenModelOption = !isOpenModelOption;
  } else {
    // close option
    console.log("closing ...");
    isOpenModelOption = !isOpenModelOption;
    customNetworkSection.style.display = "none";
  }
}
...
window.addEventListener("load", (e) => {
  ...
  document
    .getElementById("single-upload")
    .addEventListener("change", uploadImageToCanvas);
...
function drawImage(elementID, tempObj) {
  ...
  if (widthTemp < widthDiv - 10 && heightTemp < heightDiv - 10) {
    ...
    ctx.putImageData(
      imageData,
      (widthDiv - widthTemp) / 2,
      (heightDiv - heightTemp) / 2
    );
...
function displayJson(obj) {
  ...
  for (let i = 0; i < lengthJSON; i++) {
    if (obj[i].length > 10) {
      if (obj[i].search("Conv2d") === 1) {
        ...
        nodes.add({
          id: numOfId,
          label: "AdaptiveAvgPool2d - " + numOfId,
          shape: "box",
        });
      } else if (obj[i].search("Linear") === 1) {
  ...
  for (let i = 0; i < special.length; i++) {
    network.moveNode(
      special[i],
      network.getPosition(special[i] - 2)["x"],
      network.getPosition(special[i] - 2)["y"]
    );
  }
